...
#####################################################################
#myData <- read.csv("data/registrations.csv")
#myData$COMPETENCY <- factor(myData$COMPETENCY, levels=rev(levels(myData$COMPETENCY)))
#myData$MEASURE <- factor(myData$MEASURE, levels=rev(levels(myData$MEASURE)))
```
...
A total of `r length(myData$COMPETENCY)` registrations have been completed as of `r format(Sys.Date(),format="%B %d, %Y")`. This includes a total of `r length(unique(myData$COURSE))` different courses. The distribution of the registrations between the eight core competencies is shown in figure \@ref(fig:competency).
...
<question>Do you think that there are too many exams in this mix? What is the desired balance between exams, products, and performances for General Education?</question>
